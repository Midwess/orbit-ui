...
function CreativeChatDemo() {
  ...
  return <div className="u-wo-chat-demo"><header><div><span className="u-wo-avatar">A</span><span><b>3D animation for Geex</b><small><i /> Writing…</small></span></div><strong>Work OS</strong></header><div className="u-wo-chat-feed" aria-live="polite"><div className="u-wo-incoming"><span className="u-wo-avatar">G</span><p>Can we turn this direction into a short launch video?</p></div><figure><img src="/work-os/editorial-01.jpeg" width="1152" height="864" alt="Editorial beauty campaign close-up" /><img src="/work-os/editorial-02.jpeg" width="1152" height="864" alt="Editorial headphone portrait" /><figcaption><span>🔥 13</span><span>🥲 8</span><span>✦ Selected</span></figcaption></figure>{messages.map((message, index) => <p className="u-wo-self" key={`${message}-${index}`}>{message}</p>)}</div><form onSubmit={send}><input name="work-os-message" autoComplete="off" value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="Text message…" aria-label="Work OS message" /><button type="submit">Send message</button></form></div>
}
...
function EditorialGalleryDemo() {
  ...
  const image = selected === 'beauty' ? '/work-os/editorial-01.jpeg' : '/work-os/editorial-02.jpeg'
  return <div className="u-wo-gallery-demo"><header><span>Creative files</span><b>Campaign 04</b></header><div className="u-wo-gallery-stage"><img src={image} width="1152" height="864" loading="lazy" decoding="async" alt={selected === 'beauty' ? 'Selected beauty campaign asset' : 'Selected portrait campaign asset'} /><span>Selected · 01</span></div><div className="u-wo-gallery-thumbs"><button className={selected === 'beauty' ? 'is-active' : ''} aria-pressed={selected === 'beauty'} onClick={() => setSelected('beauty')}><img src="/work-os/editorial-01.jpeg" width="1152" height="864" loading="lazy" decoding="async" alt="Choose beauty campaign asset" /></button><button className={selected === 'portrait' ? 'is-active' : ''} aria-pressed={selected === 'portrait'} onClick={() => setSelected('portrait')}><img src="/work-os/editorial-02.jpeg" width="1152" height="864" loading="lazy" decoding="async" alt="Choose portrait campaign asset" /></button></div></div>
}
