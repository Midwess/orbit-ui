...
import { Avatar, AvatarGroup, Badge, IconAction, SectionTitle, Surface, UnifiedButton, ViewHeading } from '../components/UnifiedPrimitives'
...
export function ChatBlock() {
  ...
  return <div className="u-view u-chat-view">
    <ViewHeading eyebrow="Communication" title="One place to decide and act" description="Bring people, operational signals, creative work, and your AI partner into the same conversation." action={<UnifiedButton tone="accent" icon="plus" onClick={createThread}>New conversation</UnifiedButton>} />
    <div className="u-chat-layout">
      ...
      <Surface className="u-conversation-panel">
        <header className="u-conversation-head"><div><span className={`u-thread-symbol u-thread-symbol--${selected.tone}`}>{selected.title[0]}</span><span><b>{selected.title}</b><small><i /> {invited ? '5' : '4'} people · Orbit AI active</small></span></div><div><IconAction label={pinned ? 'Unpin conversation' : 'Pin conversation'} icon="map" aria-pressed={pinned} onClick={() => setPinned((current) => !current)} /><UnifiedButton tone="quiet" size="compact" icon="users" onClick={() => setInvited((current) => !current)}>{invited ? 'Invited' : 'Invite'}</UnifiedButton></div></header>
      ...
      <aside className="u-chat-context">
        ...
        <Surface className="u-people-block"><SectionTitle title="People" meta="4 in this thread" /><AvatarGroup people={[{ name: 'Hanna', color: '#ff7c6e' }, { name: 'Maya', color: '#8c89ff' }, { name: 'Noah', color: '#a9e7f4' }, { name: 'Orbit', color: '#ffd36b' }]} /></Surface>
