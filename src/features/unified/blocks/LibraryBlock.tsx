...
import { Avatar, AvatarGroup, Badge, IconAction, ProgressBar, SectionTitle, Surface, UnifiedButton, ViewHeading } from '../components/UnifiedPrimitives'
...
export function LibraryBlock({ theme, onTheme }: { theme: 'dark' | 'light'; onTheme: () => void }) {
  ...
  return <div className="u-view u-library-view">
    <ViewHeading eyebrow="Design system" title="Components made to compose" description="The merged language uses generous geometry, editorial color, clear hierarchy, and controls with an obvious purpose." action={<UnifiedButton tone="accent" icon={theme === 'dark' ? 'sun' : 'moon'} onClick={onTheme}>Use {theme === 'dark' ? 'light' : 'dark'} theme</UnifiedButton>} />
